...
(function () {
  ...
  const esc = (s) => String(s == null ? "" : s).replace(/[&<>"']/g, (c) =>
    ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c]));
  const colsForWidth = (w) => (w < 520 ? 2 : w < 820 ? 3 : w < 1080 ? 4 : 5);
  ...
  function renderRack(host, items, mode) {
    ...
    const sizes = balancedRows(items.length, colsForWidth(w));
    let idx = 0, html = "";
    for (const size of sizes) {
      ...
      html += `<div class="rack"><div class="rk-tips" style="grid-template-columns:${cols}">${tips}</div>` +
        `<div class="rk-rod"></div><div class="rk-hangers" style="grid-template-columns:${cols}">${hangs}</div>` +
